...
function formatTime(minutes, seconds) {
  ...
  return m.concat(':', seconds.toString().padStart(2, '0'));
}
...
function isPalindrome(str) {
  const tmp = str
    ...
    .reverse()
    .join('');
  return tmp === str.toLowerCase().replace(/[^a-z]/g, '');
}
...
function findLongestWord(sentence) {
  const tmp = sentence.split(' ').sort((a, b) => b.length - a.length);
  return tmp[0];
}
...
function encodeToRot13(str) {
  ...
  let tmp1 = 0;
  for (let i = 0; i < str.length; i += 1) {
    tmp1 = tmp[i].charCodeAt();
    if (tmp1 >= 65 && tmp1 <= 90) {
      if (tmp1 >= 65 && tmp1 <= 77) {
        tmp1 += 13;
      } else tmp1 -= 13;
    } else if (tmp1 >= 97 && tmp1 <= 122) {
      if (tmp1 >= 97 && tmp1 <= 109) {
        tmp1 += 13;
      } else tmp1 -= 13;
    }
    tmp[i] = String.fromCharCode(tmp1);
  }
  return tmp.join('');
}
